...
// Start with empty users array - no conflicts
let users = [];
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (path === '/api/v1/auth/register' && req.method === 'POST') {
      ...
      // Allow duplicate emails - no conflict checking
      const newUser = {
        id: users.length + 1,
        email: body.email,
...
server.listen(PORT, () => {
  console.log(`\n🚀 FINAL PHARMACY SERVER STARTED`);
  console.log(`📡 Server: http://localhost:${PORT}`);
  console.log(`🔗 Health: http://localhost:${PORT}/health`);
  console.log(`\n✅ NO EMAIL CONFLICTS - REGISTRATION ALWAYS WORKS!`);
  console.log(`\n👤 LOGIN CREDENTIALS:`);
  console.log(`   Use ANY email/password to register`);
  console.log(`   Then login with same credentials`);
